  ...
  if (isSubmitted && score !== null) {
    return (
      <Box sx={{ backgroundColor: 'background.default', minHeight: '100vh', p: 5 }}>
        ...
        <Paper sx={{ p: 4, mb: 3 }}>
          <Typography variant="h6">Your Score: {score} / {quiz.totalMarks}</Typography>
          <Typography variant="body1" sx={{ mt: 2 }}>
            Date: {new Date().toLocaleString()}
          </Typography>
        </Paper>

        <Button variant="contained" onClick={() => navigate('/student/quizzes')} sx={{ mt: 2 }}>
          Back to Quizzes
        </Button>
      </Box>
    );
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
              {quiz.questions.map((question, index) => (
                <Button
                  ...
                  color={answers[question.id] !== undefined ? 'primary' : 'default'}
        ...
        <Grid item xs={12} md={9}>
          <Paper sx={{ p: 4 }}>
            ...
            <RadioGroup
              ...
              onChange={(e) => handleOptionChange(currentQuestion.id, parseInt(e.target.value, 10))}
            >
